#!/usr/bin/env python3
"""Train on detailed Zulrah with rotations"""
import sys
...
def main():
    ...
    run_name = f"zulrah_detailed_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    save_dir = f"models/{run_name}"
    os.makedirs(save_dir, exist_ok=True)
    
    print("="*60)
    print("ZULRAH DETAILED TRAINING")
    print("  - 4 rotation patterns")
    print("  - Position tracking (east/west/pillar)")
    print("  - Gear switching (range/mage)")
    print("  - Prayer switching per phase")
    print("  - Venom clouds & snakelings")
    print("="*60)
